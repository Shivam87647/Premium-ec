...
### Color Palette
- Primary background: #FAFAFA (warm of-white). Secondary background: #F5F5F0
(cream).
...
24px on mobile.- Section vertical padding: 80–120px. Never less than 64px between major sections.
...
0.1s delay.
3. **Add-to-cart efect**: When a user clicks "Add to Cart", animate a ghost image of the
...
(scale 1→1.3→1, type: "spring", stifness: 400). A small "+1" badge should pop in with a
...
- Buttons must have visible focus-visible outlines (2px ofset ring in accent color) for
accessibility.
...
widths.- Empty states must have illustrations and helpful CTAs (e.g., empty cart shows illustration +
...
word-by-word stagger reveal. CTA button with shimmer efect. Auto-rotating carousel of 3–4
...
- **Quantity selector**: Styled increment/decrement with min 1 and max = stock count.- **Add to Cart button**: Full-width, large, with the flying-image animation described above.
...
pending=yellow, shipped=blue, delivered=green, cancelled=red). Click to view order detail.- **Order Detail**: Full breakdown with items, shipping tracking info, status timeline
animation.
...
lightweight RTE like TipTap).- Category selector (from categories table), tags (multi-select/creatable).
...
add internal notes).
### Coupon / Discount Management (/admin/coupons)- CRUD coupons: Code, type (percentage/fixed amount), value, minimum order amount,
...
footer with icons.
- **Announcement Bar**: Toggle on/of, text content, link URL, background color. Shows as a
...
- Orders over time.
- Top products by revenue and units.- Top categories.
...
created_at.16. **coupons** — id, code (unique), type (enum: 'percentage', 'fixed'), value (numeric),
...
/search/page.tsx
/about, /contact, /faq, /policies.../(admin)
...
### Razorpay Integration:- Create Payment Intent on the server (API route) when user initiates checkout.
